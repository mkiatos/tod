    ...
    for(int i=0;i<depth.rows;i++)
    {
        for(int j=0;j<depth.cols;j++)
        {
            ...
            if(depth_value != 0)
            {
                ...
                cloud->points[i * rgb.cols + j].x = x;
                cloud->points[i * rgb.cols + j].y = y;
                cloud->points[i * rgb.cols + j].z = z;
                ...
                cloud->points[i * rgb.cols + j].rgb = *(float *)(&color);
            }
        }
    }
}
...
void depthToPointCloud(const cv::Mat &depth, const PinholeCamera &camera,
    ...
    for(int i=0;i<depth.rows;i++)
    {
        for(int j=0;j<depth.cols;j++)
        {
            ...
            if(depth_value != 0)
            {
                ...
                cloud->points[i * depth.cols + j].x = x;
                cloud->points[i * depth.cols + j].y = y;
                cloud->points[i * depth.cols + j].z = z;
            }
        }
    }
}
